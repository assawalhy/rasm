...
export default function canvasEvents() {
  ...
  const startTransFunc = (e) => {
    ...
    if (subTools.type === 'move') {
      ...
      if (distTo.x < d && distTo.y < d) {
        ...
        let o = new vector(
          math.constrain(sketch.gs.center.x, 0, sketch.gs.width),
          math.constrain(sketch.gs.center.y, 0, sketch.gs.height),
        );
  ...
  function scaleAxises(e) {
    if (subTools.axis === 'x') {
      ...
      const xEq = lines.lineEquation(-sketch.gs.transform.xAngle, sketch.gs.center);
      const incre =
        (math.dist(mouse.x, mouse.y, rotatedMouse.x, rotatedMouse.y) ** 2 -
