...
SUI.DropDown = (options = {}) => {
  const refineDDlocation = (dd, toggleLocation) => {
    /////// for the cloned DropDown ////////
    /////// for the cloned DropDown ////////
    /////// for the cloned DropDown ////////
    /////// for the cloned DropDown ////////

    const location = dd.location;
    const indSize = {
      width: dd.indicator.offsetWidth,
      height: dd.indicator.offsetHeight,
    };
    let indLocation = { x: 0, y: 0 };
    const toggleSize = {
    ...
    const style = window.getComputedStyle(dd.menu);
    const margin = [
      Number.parseInt(style.marginLeft.replace('px', '')),
      Number.parseInt(style.marginTop.replace('px', '')),
      Number.parseInt(style.marginRight.replace('px', '')),
      Number.parseInt(style.marginBottom.replace('px', '')),
    ];
    ...
    switch (dir) {
      case 'top':
        indLocation = {
          x: constrain(toggleLocation.x - width / 2, newLocation.x + 5, newLocation.x + menuSize.width - width - 5),
      ...
      case 'right':
        indLocation = {
          ...
          y: constrain(toggleLocation.y - height / 2, newLocation.y + 5, newLocation.y + menuSize.height - height - 5),
        };
        break;
      case 'left':
        indLocation = {
  ...
  const setClonedDDstyles = (dd) => {
    const rect = dd.toggle.getBoundingClientRect();
    const location = {
      x: rect.left,
      y: rect.top,
    };
  ...
  const obj = {
    ...
    prepare: function () {
      ...
      for (const evt of options.toggleEvents) {
