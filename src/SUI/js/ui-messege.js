SUI.TempMessege = (options = {}) => {
  ...
  const me = {
    ...
    show: function () {
      ...
      if (this.status === 'hiding' || this.status === 'hidden') {
        ...
        const jelt = $(this.elt); // this is the same as me
    ...
    hide: function (closing = false) {
      const onhoverFix = this.options.onhoverFix;
    ...
    __setBox: function () {
      ...
      if (pos) {
        if (pos.top || pos.top === 0) {
          this.elt.style.top = `${pos.top}px`;
        } else {
          this.elt.style.top = '';
        }

        if (pos.left || pos.left === 0) {
          this.elt.style.left = `${pos.left}px`;
        } else {
          this.elt.style.left = '';
        }

        if (pos.bottom || pos.bottom === 0) {
          this.elt.style.bottom = `${pos.bottom}px`;
        } else {
          this.elt.style.bottom = '';
        }

        if (pos.right || pos.right === 0) {
          this.elt.style.right = `${pos.right}px`;
        } else {
  ...
  if (options.duration <= 0 || me.fixed) {
    me.fixed = true;
    me.elt.classList.add('fixed');
  } // setting fixed to true
  if (options.toolsbar) {
