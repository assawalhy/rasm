...
class ArithmeticSequence extends Node {
  ...
  calculate(cs, tempVars) {
    ...
    throw new OutOfRangeException(
      `Index ${cs.vars[3].value.calculate(cs)} is out of the range ${`[${0},${Count(cs, tempVars) - 1}]`}`,
    );
  }

  length(cs, tempVars) {
    return Math.floor(
      (end.calculate(cs, tempVars) - this.start.calculate(cs, tempVars)) / this.step.calculate(cs, tempVars) + 1,
    );
  }
  ...
  isEqual(node) {
    if (this.constructor === node.constructor) {
      const arithmeticSequence = node;
      if (
        this.start.isEqual(arithmeticSequence.start) &&
        this.end.isEqual(arithmeticSequence.end) &&
        this.step.isEqual(arithmeticSequence.step)
      ) {
        return true;
      }
    }
