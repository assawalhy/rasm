...
class Round extends Node {
  ...
  derivative(cs) {
    return new If(
      new Equals(new Mod(new Multiply(this.children[0], new Constant(2)), new Constant(1)), new Constant(0)),
      new Constant(Number.NaN),
      new Constant(0),
    );
  }

  simplify() {
    const sChild = this.children[0].simplify();
