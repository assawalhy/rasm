...
class Log extends Node {
  ...
  derivative(cs) {
    return new Multiply(
      new Log(new Constant(Math.E), this.children[1]),
      new Divide(this.children[0].derivative(cs), this.children[0]),
    );
  }
  ...
  isEqual(node) {
    ...
    if (node.type === 'Ln') {
      if (this.children[1].simplify().isEqual(new Constant(Math.E)))
        if (this.children[0].simplify().isEqual(node.children[0].simplify())) return true;
    }
    return false;
  }
  ...
  toString() {
    ...
    if (this.children[1] instanceof Constant && children[1].value === Math.E) {
