...
class Max extends Node {
  ...
  calculate(cs, tempVars) {
    let max = this.items[0].calculate(cs, tempVars);
    let v = 0;
  ...
  derivative(cs) {
    if (this.children.length === 2) {
      return new If(
        new GreaterThan(this.children[0], this.children[1]),
  ...
  sub_process_1(index, cs) {
    if (index < this.children.length - 1) {
      return new If(
        new Equals(this, this.children[index]),
        this.children[index].derivative(cs),
        sub_process_1(index + 1, cs),
      );
    }
  ...
  isEqual(node) {
    if (this.constructor === node.constructor) {
      if (this.children[0].isEqual(node.children[0])) return true;
    }
    return false;
  }
