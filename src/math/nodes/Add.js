...
class Add extends Node {
  ...
  simplify() {
    ...
    if (sChild2.isEqual(new Constant(0))) {
      return sChild1;
    }
    if (sChild1 instanceof Constant && sChild2 instanceof Constant) {
      return new Constant(sChild1.value + sChild2.value);
    }
    if (sChild1 instanceof Neg) {
  ...
  isEqual(node) {
    if (!this.containsVariable) {
      if (!node.containsVariable) {
        ...
        if (this.calculate(cal) === node.calculate(cal)) {
          return true;
        }
      } else return false;
