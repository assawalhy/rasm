...
function __tex2max() {
    ...
    function lex(lexemes) {
        ...
        function startLexing() {
            ...
            while (index < lexemes.length) {
                ...
                const tokenType = findMatchingTokenTypeByString(lexemes[index]);
                tokenValue = consume();
    ...
    function isTrigonometricFunction(func) {
        const name = TRIGONOMETRIC_FUNCTIONS.find((e) => e.name === func);
        const inverse = TRIGONOMETRIC_FUNCTIONS.find((e) => e.inverse === func);
    ...
    function getInverseTrigonometricFunction(func) {
        ...
        let inverseTrig;
        for (const key in TRIGONOMETRIC_FUNCTIONS) {
    ...
    const environments = ['matrix', 'pmatrix', 'bmatrix', 'Bmatrix', 'vmatrix', 'Vmatrix', 'smallmatrix'];
    ...
    function isGreekLetter(letter) {
        const symbol = letters.find((e) => e.name === letter.toLowerCase());
        const name = letters.find((e) => e.symbol === letter.toLowerCase());
    ...
    function isDelimiter(delimiterName) {
        let isMatch = false;
        const delimiter = DELIMITERS.get(delimiterName);
    ...
    function parseLatex(tokens) {
        ...
        function lookBack(position) {
            const previousToken = structure[structure.length - position];
        ...
        function parseNumber() {
            ...
            const nextToken = peekType();
            const previousTokenValue = lookBackValue();
            const previousStructureType = lookBack(1); // Check if previously added structure is a function
            if (previousTokenValue !== '^' && previousTokenValue !== '_' && previousStructureType !== 'function') {
                if (nextToken !== null && nextToken === TOKEN_TYPES.NUMBER_LITERAL.name) {
                    ...
                    const currentNumber = parseDigit().value + parseExpression().value;
                    return {
                        type: 'number',
                        value: currentNumber,
                    };
        ...
        function parseDecimalSeparator() {
            ...
            const nextToken = peekType();
            const previousStructureType = lookBack(1);
        ...
        function isReservedWord(word) {
            ...
            for (const key in RESERVED_WORDS) {
                if (!RESERVED_WORDS.hasOwnProperty(key)) {
                    continue;
                }
                const regex = RESERVED_WORDS[key].regex;
                if (regex.test(word)) {
                    isReserved = true;
                }
            }
        ...
        function parseVariable() {
            ...
            if (isReservedWord(word)) {
                ...
                const reservedWordResult = handleReservedWord(word);
                const reservedWord = reservedWordResult.value;
                reservedWordLength = reservedWord[0].length;

                debug(
                    'reserved word: ' + reservedWord[0] + ', length: ' + reservedWordLength + ', index ' + reservedWord.index,
                );
        ...
        function parseBracket() {
            let node = null;
            const bracketName = getBracketName(getCurrentTypeSymbol());
            const bracketType = getBracketType(getCurrentTypeSymbol());
            node = {
                type: bracketType,
                symbol: bracketName,
                value: consume(),
        ...
        function parseMacro(macroName) {
            let macro = null;
            const isMacroMatch = isMacro(macroName);
            const isGreek = isGreekLetter(macroName);
        ...
        function parseEnvironment(state) {
            ...
            const environmentType = parseWord();
            skipToken(); // Skip brace
        ...
        function parseOperatorname() {
            ...
            const functionalWord = parseWord();
        ...
        function parseFunction(functionName) {
            let node = {};
            const func = getFunctionName(functionName);
            node = {
                type: 'function',
                value: func,
            };
        ...
        function startParse() {
            ...
            debug(
                '--------- End of while loop. Tokens position: ' + (index - 1) + ' of ' + (tokens.length - 1) + ' ---------',
            );
            return structure;
        }
        ...
        if (Array.isArray(parsedLatex)) {
            for (let i = 0; i < parsedLatex.length; i++) {
                if (parsedLatex[i].type === 'group') {
                    const containsGroupVariable = checkForVariable(parsedLatex[i].value);
    ...
    function stripParenthesis(mathString) {
        const openingParenthesis = mathString.charAt(0);
        const closingParenthesis = mathString.charAt(-1);
    ...
    function handleMatrix(parsedLatex) {
        ...
        const matrixRowSize = matrixArray[0].length;
    ...
    function findIntegralEnd(tokens) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (tokens[i].type === 'function' && tokens[i].value === 'integral') {
                ...
                const regex = /(d)[A-z]/g; // Match integration ends like dx and dy in dxdy
    ...
    function handleLowerSumArguments(parsedLatex) {
        ...
        const upperLim = parsedLatex.slice(2);
        let value = '';
    ...
    function handleLimitArguments(limitArgs) {
        if (!checkForVariable(limitArgs[0])) {
        ...
        const variable = limitArgs[0].value;
        const upperLim = limitArgs.slice(2);
        let value = '';

        value += transpiler(wrapForTranspilation(upperLim));

        const direction = isOneSidedLimit(limitArgs.slice(2));
    ...
    function isOneSidedLimit(expression) {
        ...
        for (let i = 0; i < expression.length; i++) {
            if (expression[i].type === 'group') {
                const isOneSidedGroup = isOneSidedLimit(expression[i].value);
        ...
        for (index; index < parsedLatex.length; index++) {
            ...
            function doToken() {
                ...
                if (isMacro(item.value)) {
                    const macro = MACROS.get(item.value);
                    if (macro === null) {
            ...
            function doFunction() {
                ...
                if (item.value === 'sqrt') {
                    if (parsedLatex[index + 1].type === 'group') {
                        ...
                        if (parsedLatex[index + 1].symbol === 'square' && parsedLatex[index + 2].type === 'group') {
                            ...
                            const nthArgString = transpiler(parsedLatex[index + 1].value);
                ...
                function handleNormalFunction() {
                    ...
                    if (parsedLatex[index].type === 'group') {
                        ...
                        let { expressionLength } = getExpressionLength(
                            parsedLatex.slice(index + 1),
                            ['function'],
                            ['+', '-', '+-'],
                        );
                        expressionLength += 1;
                        ...
                        const latexSlice = parsedLatex.slice(index);
                ...
                function handleTrig() {
                    ...
                    if (parsedLatex[index + 1].value === '^') {
                        ...
                        if (stripAllParenthesis(exponent) === '-1') {
                            ...
                            const inverseFunc = getInverseTrigonometricFunction(func);
                            if (inverseFunc !== null) {
                                func = inverseFunc;
                            } else {
                    ...
                    if (parsedLatex[index + 1].type === 'group') {
                        ...
                        let { expressionLength } = getExpressionLength(
                            parsedLatex.slice(index + 2),
                            ['function'],
                            ['+', '-', '+-'],
                        );
                        expressionLength += 1;

                        expression += transpiler(wrapForTranspilation(parsedLatex.slice(index + 1, index + 1 + expressionLength)));
                        index += expressionLength - 1;
                    } else {
                        const { expressionLength } = getExpressionLength(
                            parsedLatex.slice(index + 1),
                            ['function'],
                            ['+', '-', '+-'],
                        );
                ...
                function handleBinomial() {
                    ...
                    const expr1 = parsedLatex[index + 1].type;
                    const expr2 = parsedLatex[index + 2].type;
                    ...
                    binomialString += buildMaximaFunctionString('binomial', expression1, expression2);
                ...
                function handleLimit() {
                    ...
                    if (parsedLatex[index + 1].value === '_' && parsedLatex[index + 2].type === 'group') {
                        ...
                        if (typeof parsedLatex[index + 3] !== 'undefined') {
                            const { expressionLength: limitLength } = getExpressionLength(
                                parsedLatex.slice(index + 3),
                                [],
                                ['+', '-', '+-'],
                            );
                ...
                function handleSum() {
                    ...
                    if (parsedLatex[index + 1].value !== '_' && parsedLatex[index + 1].value !== '^') {
                        ...
                        const integrationLimits = handleUpperAndLowerArgsSum(parsedLatex.slice(index + 1));
                    ...
                    if (typeof parsedLatex[index + 1] !== 'undefined') {
                        const { expressionLength: sumLength } = getExpressionLength(
                            parsedLatex.slice(index + 1),
                            [],
                            ['+', '-', '+-'],
                        );
                    ...
                    sumString += buildMaximaFunctionString('sum', expression, indexVariable, lowerArgAssignment, upperArg);
                ...
                function handleIntegral() {
                    ...
                    if (parsedLatex[index + 1].value !== '_' && parsedLatex[index + 1].value !== '^') {
                        ...
                        // Finite integral
                        const integralArgs = parsedLatex.slice(index + 1, index + 5);
                        const integrationLimits = handleUpperAndLowerArgs(integralArgs);
                    ...
                    const integralEnd = findIntegralEnd(parsedLatex.slice(index));

                    integrationVariable += integralEnd.variable;
                    integralLength = integralEnd.length;

                    const unTranspiledIntegralLatex = parsedLatex.slice(index, index + integralLength);
                    assertNotUndefined(
                        unTranspiledIntegralLatex[unTranspiledIntegralLatex.length - 1],
                        'Missing argument in integral',
                    );
                    ...
                    if (isSymbolic) {
                        integralString += buildMaximaFunctionString('integrate', expression, integrationVariable);
                    } else {
            ...
            function doEnvironment() {
                if (item.state === 'begin') {
                    ...
                    let expression = '';
                    const envLength = environmentLength(parsedLatex.slice(index));
    ...
    function postParse(parsedLatex) {
        ...
        function parseGroup() {
            const groupName = getCurrentItem().symbol;
            const length = matchingBracketLength(parsedLatex.slice(index), null, groupName);
        ...
        function parseFloat() {
            ...
            if (peekType(2) === 'number') {
                ...
                const decimal_separator = peekValue(1);
        ...
        function createGroup() {
            ...
            const length = findGroupLength(parsedLatex.slice(index), delimiter, value);
        ...
        function findGroupLength(items, delimiter, symbol) {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (item.type === 'delimiter' && item.value === startDelimiter && nextItemValue === symbol) {
                    depth++;
                    debug('-- Found starting point, depth ' + depth);
                } else if (item.type === 'delimiter' && item.value === endDelimiter && nextItemValue === symbol) {
                    if (depth === 1) {
                        debug('-- Found end of symbol group at position ' + i);
                        return i;
                    }
                    depth--;
        ...
        function matchingBracketLength(items, delimiter, bracketType) {
            ...
            if (delimiter) {
                for (let i = 0; i < items.length; i++) {
                    ...
                    if (item.type === 'delimiter' && item.value === startDelimiter && nextItemValue === startBracket) {
                        bracketDepth++;
                        debug('-- Found starting bracket, depth ' + bracketDepth);
                    } else if (item.type === 'delimiter' && item.value === endDelimiter && nextItemValue === endBracket) {
                        if (bracketDepth === 1) {
                            debug('-- Found original closing bracket at position ' + i);
                            return i;
                        }

                        bracketDepth--;
                        debug('-- Found closing bracket, depth ' + bracketDepth);
                    }
                }
            } else {
                for (let i = 0; i < items.length; i++) {
    ...
    class TeX2Max {
        ...
        toMaxima(latex) {
            ...
            const scannerResult = scan(latex);
            debug(scannerResult);

            const lexerResult = lex(scannerResult);
            debug(lexerResult);

            const parsedLatex = parseLatex(lexerResult);
            debug(parsedLatex);

            this.structure = postParse(parsedLatex);
            debug(this.structure);

            const transpiledExpression = transpiler(this.structure);
...
export const tex2max = __tex2max();
