...
var Fragment = P(function(_) {
  _.init = function(withDir, oppDir, dir) {
    ...
    pray('no half-empty fragments', (withDir && oppDir) || (!withDir && !oppDir));
...
Controller.open(function(_) {
  Options.p.substituteTextarea = function() {
    return $('<textarea autocapitalize=off autocomplete=off autocorrect=off ' +
      'spellcheck=false x-palm-disable-ste-all=true ' + (window.disableNativeKeypad ? "readonly" : '') + '/>')[0];
  };
  ...
  _.editablesTextareaEvents = function() {
    ...
    this.container.on('cut', function(e) {
      if (cursor.selection) {
        setTimeout(function() {
...
var Embed = LatexCmds.embed = P(Symbol, function(_, super_) {
  ...
  _.parser = function() {
    var self = this;
    let string = Parser.string, regex = Parser.regex, succeed = Parser.succeed;
    return string('{').then(regex(/^[a-z][a-z0-9]*/i)).skip(string('}'))
      .then(function(name) {
